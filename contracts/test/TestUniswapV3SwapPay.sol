...
contract TestUniswapV3SwapPay is IUniswapV3SwapCallback {
    ...
    function uniswapV3SwapCallback(int256, int256, bytes calldata data) external override {
        ...
        if (pay0 > 0) {
            IERC20Minimal(IGMeowFiV3Pool(msg.sender).token0()).transferFrom(sender, msg.sender, uint256(pay0));
        } else if (pay1 > 0) {
            IERC20Minimal(IGMeowFiV3Pool(msg.sender).token1()).transferFrom(sender, msg.sender, uint256(pay1));
        }
    }
}
